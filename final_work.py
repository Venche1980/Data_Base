...
def next_cards_handler(message):
    ...
    try:
        conn = connect_db()
        cur = conn.cursor()
        cur.execute("SELECT word_en, word_ru FROM Words ORDER BY RANDOM() LIMIT 1")
        result = cur.fetchone()
        if result is None:
            raise Exception("No words found in the database.")
...
def message_reply(message):
    text = message.text
    markup = types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    if userStep.get(message.chat.id) == 1:
        try:
            ...
            cur.execute("INSERT INTO UserWords (user_id, word_en, word_ru) VALUES (%s, %s, %s)",
                        (message.from_user.id, word_en, word_ru))
            conn.commit()
            cur.close()
            conn.close()
            cur = connect_db().cursor()
            cur.execute("SELECT COUNT(*) FROM UserWords WHERE user_id = %s", (message.from_user.id,))
            word_count = cur.fetchone()[0]
            bot.send_message(message.chat.id, f"Слово '{word_en}' добавлено с переводом '{word_ru}'! Теперь вы изучаете {word_count} слов.")
            userStep[message.chat.id] = 0
            next_cards_handler(message)
        except ValueError:
            bot.send_message(message.chat.id, "Ошибка! Введите слово и перевод через запятую, например: cat, кот.")
        except Exception as e:
            bot.send_message(message.chat.id, f"Ошибка при добавлении слова в базу данных: {e}")
        return
